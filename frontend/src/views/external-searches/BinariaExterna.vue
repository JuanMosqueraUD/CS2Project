...
// BÚSQUEDA BINARIA EXTERNA: Encuentra el bloque correcto y luego busca dentro usando búsqueda binaria
function buscarElemento(elemento: number) {
  for (let i = 0; i < estructura.value.length; i++) {
    ...
    if (ultimoElemento === null || elemento <= ultimoElemento) {
      ...
      const posicion = busquedaBinariaEnBloque(bloque, elemento);
      if (posicion !== -1) {
        return { encontrado: true, bloque: i, posicion: posicion };
      }
      // Si no está en este bloque, no está en ninguno
...
// Búsqueda binaria dentro de un bloque específico
function busquedaBinariaEnBloque(bloque: (number | null)[], elemento: number): number {
  // Encontrar el último elemento válido (no null) para determinar el rango
  ...
  if (ultimoIndiceValido === -1) {
    return -1; // Bloque vacío
  }
  
  let izquierda = 0;
  let derecha = ultimoIndiceValido;
  
  while (izquierda <= derecha) {
    const medio = Math.floor((izquierda + derecha) / 2);
    const valorMedio = bloque[medio] as number; // Sabemos que no es null en este rango
...
function eliminar() {
  ...
  const num = parseInt(valor.value);
  // Usar búsqueda binaria para encontrar el elemento a eliminar
  const resultadoBusqueda = buscarElemento(num);
...
function exportarEstructura() {
  ...
  const exportData = createExportData('binaria-externa', config, estructura.value);
  const filename = generateExportFileName('binaria-externa');
...
function importarEstructura(event: Event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const validation = validateExternalBinaryImport(importData);
