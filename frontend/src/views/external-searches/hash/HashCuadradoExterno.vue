...
const hashCuadradoBloque = (clave: number): number => {
  ...
  if (isNaN(hash) || hash < 0 || hash >= numeroBloques.value) {
    // Usar hash módulo como respaldo
...
const insertar = () => {
  ...
  const bloqueHash = hashCuadradoBloque(num);
...
function eliminar() {
  ...
  if (eliminacionExitosa) {
    ...
    errorMessage.value = `Eliminado del Bloque ${hashCuadradoBloque(num) + 1} (${ubicacion}). (${elementosInsertados.value}/${capacidad.value})`;
    valor.value = "";
  } else {
